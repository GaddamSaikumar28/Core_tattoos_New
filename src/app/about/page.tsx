...
// =========================================================
// 🚀 FIX: Fixed publish date for the About page's Article schema.
// This page's content is edited via the Shopify metaobject, which has no
// "published at" field exposed today. Using `new Date()` at request time
// was a bug — it silently changed the reported publish date on every
// server render/revalidation. Set this once to the real date this page's
// content went live, or wire it to a metaobject field if one gets added
// (see the queries/index.ts note further down for how to do that).
// =========================================================
const ABOUT_PAGE_PUBLISHED_DATE = '2025-01-01T00:00:00Z';

// =========================================================
// 2. MAIN ABOUT US SERVER COMPONENT
// =========================================================
export default async function AboutUs() {
  ...
  // =========================================================
  // 🚀 SEO: INJECT ALL ABOUT & CONTACT SCHEMAS
  // =========================================================
  const siteUrl = process.env.NEXT_PUBLIC_SITE_URL || 'https://www.justtattoos.com';
  ...
  const schemas: any[] = [
    {
      ...
      "sameAs": ["https://instagram.com/justtattoos"],
      "address": {
        "@type": "PostalAddress",
    ...
    },
    {
      "@context": "https://schema.org",
      "@type": "Article",
      "@id": `${siteUrl}/about#article`,
      "headline": content.heroTitle || "About",
      "url": `${siteUrl}/about`,
      "author": { "@type": "Person", "name": "Just Tattoos Team" },
      "publisher": {
        "@type": "Organization",
        "name": "Just Tattoos",
        "logo": { "@type": "ImageObject", "url": publisherLogo }
      },
      "image": content.heroImage ? [content.heroImage] : [],
      // 🚀 FIX: was `new Date().toISOString()` (recomputed on every request).
      "datePublished": ABOUT_PAGE_PUBLISHED_DATE
    },
    {
      "@context": "https://schema.org",
      "@type": "PostalAddress",
    ...
    // 🚀 FIX: removed the ContactPoint node that was here — per the client's
    // spreadsheet, ContactPoint belongs to the Contact page, not About.
    // About Page's required set is exactly: Organization, BreadcrumbList,
    // AboutPage, Article, PostalAddress, ImageObject (added below).
  ];
  ...
  if (content.heroImage) {
    schemas.push({
      "@context": "https://schema.org",
      "@type": "ImageObject",
      "contentUrl": content.heroImage,
      "url": content.heroImage,
      "caption": content.heroTitle || "About",
      "width": "1200",
      "height": "800"
    });
  }
