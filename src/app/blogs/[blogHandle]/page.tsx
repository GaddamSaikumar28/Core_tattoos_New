...
export default async function BlogListingPage({ params }: Props) {
  ...
  const articles = blog.articles?.edges?.map((edge: any) => edge.node) || []; // 🚀 FIX: defensive fallback
  const siteUrl = process.env.NEXT_PUBLIC_SITE_URL || "https://www.justtattoos.com";
  const blogUrl = `${siteUrl}/blogs/${blogHandle}`;

  // =========================================================
  // 🚀 SEO FIX: GENERATE DYNAMIC JSON-LD SCHEMAS
  // =========================================================
  
  // 1. Dynamic ItemList mapping for the articles inside this category
  const dynamicListItems = articles.map((article: any, index: number) => ({
    "@type": "ListItem",
    "position": index + 1,
  ...
  // 2. Dynamic BlogPost mapping — these are real individual articles, so
  // "@type": "BlogPosting" is correct here (unlike the Blogs Index page,
  // which lists categories and correctly uses "@type": "Blog" instead).
  const dynamicBlogPosts = articles.map((article: any) => ({
    "@type": "BlogPosting",
  ...
  const schemas = [
    {
      // BreadcrumbList Schema (3 Levels deep)
      ...
      "itemListElement": [
        ...
        },
        {
          "@type": "ListItem",
          "position": 2,
          // 🚀 FIX: was "Blogs" (plural). The client's spreadsheet sample for
          // this exact page type ("Blog Category Page") uses "Blog"
          // (singular) at this position — matches now.
          "name": "Blog",
          "item": `${siteUrl}/blogs`
        },
        {
