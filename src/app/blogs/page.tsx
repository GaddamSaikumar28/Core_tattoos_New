...
export default async function AllBlogsPage() {
  const blogs = (await getBlogs()) || []; // 🚀 FIX: defensive fallback if the fetch ever returns null/undefined
  const siteUrl = process.env.NEXT_PUBLIC_SITE_URL || 'https://www.justtattoos.com';

  // =========================================================
  // 🚀 SEO FIX: GENERATE DYNAMIC JSON-LD SCHEMAS
  // =========================================================
  
  // Build the dynamic ItemList mapping for the available blog categories
  const dynamicListItems = blogs.map((blog: any, index: number) => ({
    "@type": "ListItem",
    "position": index + 1,
  ...
  // Build the dynamic Blog mapping
  // Note: each entry here is a Shopify "blog" (a category, e.g. "News"), not an
  // individual post — so "@type": "Blog" is the correct type (Blog IS-A
  // CreativeWork, which is what BlogPosting-list properties expect). The
  // Blog Category page below correctly types its own list as "BlogPosting"
  // since it lists actual articles, not categories.
  const dynamicBlogPosts = blogs.map((blog: any) => ({
    "@type": "Blog",
    "headline": blog.title,
    "url": `${siteUrl}/blogs/${blog.handle}`,
    "description": blog.seo?.description || `Explore posts in the ${blog.title} category.`
  }));

  // Bundle the schemas precisely per the requested XLSX document mappings
  const schemas = [
    {
    ...
    },
    {
      ...
      "@context": "https://schema.org",
      "@type": "ItemList",
      "@id": `${siteUrl}/blogs#itemlist`,
      "name": "Blogs",
      "itemListElement": dynamicListItems
    },
    {
      // 3. Blog Schema
      "@context": "https://schema.org",
      ...
      "blogPost": dynamicBlogPosts
    }
  ];

  return (
    <>
      {/* Inject Consolidated Schemas dynamically */}
      <script
      ...
      <div className="w-full min-h-screen bg-black text-white selection:bg-[#FE8204] selection:text-white pt-32 md:pt-40 pb-24 px-6 md:px-10 lg:px-20 relative overflow-hidden">
        ...
        <div className="max-w-[1300px] mx-auto relative z-10">
          ...
          {/* Breadcrumbs Container (renderSchema=false prevents duplication) */}
          <div className="mb-12 opacity-80 hover:opacity-100 transition-opacity">
            <Breadcrumbs 
              items={[{ label: 'Home', url: '/' }, { label: 'Blogs', url: '/blogs' }]} 
              renderSchema={false} 
            />
          </div>
