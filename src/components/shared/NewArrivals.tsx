...
// 4. TOGGLE LOGIC WITH URL PUSH
  const handleToggleFilter = (group: keyof ActiveFilters | 'RESET', value?: string) => {
    ...
    // NOTE: For DefaultCollection, replace `/collections/new-arrival` with `/collections/${handle}`
    // NOTE: For SalePage, replace `/collections/new-arrival` with `/sale`
  ...
  return (
    ...
    <div className="bg-slate-50 min-h-screen mt-20 overflow-x-hidden w-full">
      <SharedHeroBanner 
        title="NEW ARRIVALS"
        image={bannerImage}
      ...
      <main className="container max-w-[1400px] mx-auto px-4 py-12">
        <div className="flex flex-col lg:flex-row gap-12">
          ...
          {/* PRODUCT LISTINGS */}
          {/* 🚀 FIX: Changed flex-grow to flex-1 min-w-0 to stop inner content stretching */}
