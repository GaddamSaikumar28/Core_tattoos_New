...
interface ShowcaseCarouselClientProps {
    overline: string;
...
const CarouselCard = ({ item, mode, index, mobileHidden }: { item: FormattedProduct, mode: 'product' | 'collection', index: number, mobileHidden?: boolean }) => {
    ...
    const pseudoRandom = ((item.title.length * 7) + (index * 13)) % 100 / 100;

    const rating = 4.8 + (pseudoRandom * 0.2);
    const reviewCount = 1000 + (((item.title.length * 100) + (index * 50)) % 5000);
        ...
        if (isArMode) {
            if (typeof navigator !== "undefined" && navigator.mediaDevices && navigator.mediaDevices.getUserMedia) {
                navigator.mediaDevices.getUserMedia({ video: { facingMode } })
                    ...
                    .catch((err) => {
                        logError("Camera access denied or unavailable:", err);
        ...
        <>
            ...
            >
                <Link
                    ...
                    className="relative w-full aspect-[4/5] rounded-[1.5rem] overflow-hidden bg-zinc-900 mb-4 shadow-[var(--shadow-card)] flex-shrink-0 block cursor-pointer"
                >
