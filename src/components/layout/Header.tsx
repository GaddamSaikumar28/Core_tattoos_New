...
// import { HelpCircle, Info, Sparkles, MessageCircleQuestion, Mail } from "lucide-react";
// // Add your new search API function here
...
// // Extracted variants to prevent recreation on every render
...
//   // 1. Fetch & Cache Collections
//   useEffect(() => {
//     const fetchCollections = async () => {
//       // Production tip: Check session storage to avoid refetching on every page load
...
//         sessionStorage.setItem("shopify_collections", JSON.stringify(result || []));
//       } catch (error) {
...
//       setIsSearching(true);
//       try {
//         // Calls your actual Shopify backend
...
//   useEffect(() => {
//     if (isMobileDrawerOpen || isSearchOpen) document.body.style.overflow = "hidden";
...
//               <Link href="/" className="relative z-50 flex-shrink-0 outline-none focus-visible:ring-2 focus-visible:ring-[var(--color-brand-orange)] rounded-sm transition-transform hover:scale-[1.02]">
//                 <Image
...
//               {/* Other Links omitted for brevity, keep your New Arrivals, Sale, How it Works here exactly as they were */}
...
//                     activeDropdown === "collection" || isActive("/collections") ? "text-[var(--color-brand-orange)]" : "text-gray-900"
//                   )}
//                 >
//                   COLLECTION
...
//                       animate="visible"
//                       exit="exit"
//                       className="absolute top-[calc(100%-8px)] left-1/2 -translate-x-1/2 w-[90vw] max-w-[1000px] bg-white rounded-2xl shadow-[0_30px_60px_-15px_rgba(0,0,0,0.1)] border border-gray-100 overflow-hidden z-50 origin-top cursor-default flex"
//                     >
//                       {/* Left Side: Scrollable Collections Grid */}
...
//                           /* SCROLLABLE CONTAINER: Handles unlimited collections gracefully */
//                           <div className="grid grid-cols-2 lg:grid-cols-3 gap-x-6 gap-y-4 max-h-[50vh] overflow-y-auto pr-4 
...
//                           </div>
//                         ) : (
//                            <div className="text-sm text-gray-500 py-10">No collections found.</div>
//                         )}
//                       </div>
...
//         animate="visible"
//         exit="exit"
//         // Increased width to 600px for a richer, less cramped layout
//         className="absolute top-[calc(100%-8px)] left-1/2 -translate-x-1/2 w-[600px] bg-white rounded-2xl shadow-[0_30px_60px_-15px_rgba(0,0,0,0.1)] border border-gray-100 overflow-hidden z-50 origin-top flex flex-col"
//       >
//         {/* Main Grid Section */}
...
//             <span className="text-[13px] font-medium text-gray-600">Still have questions? We're here to help.</span>
//           </div>
//           <Link 
//             href="/contact" 
//             onClick={() => setActiveDropdown(null)}
...
//             {/* Right Side Icons (Search, Cart, Profile) */}
...
//                {/* Auth / Profile mapping kept exact same as your code */}
...
//                           <button onClick={() => setIsLoggedIn(false)} className="w-full flex items-center gap-3 px-5 py-3 text-[14px] text-red-600 hover:bg-red-50  transition-colors">                               <LogOut className="w-4 h-4" /> Sign Out
//                              </button>
//                             </motion.div>
//                       )}
//                     </AnimatePresence>
...
//                             >
//                               <div className="w-14 h-14 relative bg-gray-100 rounded-xl overflow-hidden shrink-0 border border-gray-200 group-hover:border-[var(--color-brand-orange)]/50 transition-colors">
...
//                           <p className="text-sm text-gray-500 mt-2">Check the spelling or try a different term.</p>
//                         </div>
//                       )}
//                     </motion.div>
//                   )}
          ...
          <div className="hidden md:flex items-center w-full h-full">
                ...
                <AnimatePresence>
                  {activeDropdown === "how-it-works" && (
                    <motion.div
                      ...
                      animate="visible"
                      exit="exit"
                      className="absolute top-[calc(100%-8px)] left-1/2 -translate-x-1/2 w-[600px] bg-white rounded-2xl shadow-[0_30px_60px_-15px_rgba(0,0,0,0.1)] border border-gray-100 overflow-hidden z-50 origin-top flex flex-col cursor-default"
                    >
                      <div className="grid grid-cols-2 gap-2 p-6">
                        {HOW_IT_WORKS_DATA.map((item) => {
                          const Icon = item.icon;
            ...
            <div className="flex-1 flex items-center justify-end gap-3 lg:gap-5">
              ...
              >
               ...
               <div className="pl-3 lg:pl-5 border-l border-gray-200 flex items-center gap-3 lg:gap-4 h-8 relative">
                 <button 
            ...
            <div className="text-[13px] text-gray-400 py-2 text-center">
              No collections found.
                  ...
                  <div className="border-b border-gray-100">
                    ...
                    >
                      HOW IT WORKS
                      <ChevronDown className={cn("w-4 h-4 transition-transform duration-300 text-gray-400 group-hover:text-gray-900", mobileExpanded === "how-it-works" && "rotate-180")} />
                    </button>
                    <AnimatePresence>
                  ...
                  >
                    SALE 
                    <span className="text-red-500 opacity-0 group-hover:opacity-100 -translate-x-2 group-hover:translate-x-0 transition-all">→</span>
                  </Link>
                ...
                <div className="flex flex-col gap-3">
                  {isLoggedIn ? (
                    <>
                      <Link 
                        href="/account" 
                        className="flex items-center justify-center gap-2 w-full py-3.5 bg-white border border-gray-200 text-gray-900 rounded-xl font-bold hover:border-gray-300 transition-colors shadow-sm" 
                        onClick={() => setIsMobileDrawerOpen(false)}
                      >
                      ...
                      <button 
                        ...
                        onClick={() => { setIsLoggedIn(false); setIsMobileDrawerOpen(false); }}
                      >
                        <LogOut className="w-4 h-4" /> Log Out
                      </button>
                    </>
